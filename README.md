Soy Alejandro Navas Rodríguez,
estudiante Desarrollador Multiplataforma | Full Stack Junior
Sevilla, España
____________________________________________________________________________________________________________________________________________________________________________________
Soy estudiante del Grado Superior en Desarrollo de Aplicaciones Multiplataforma (DAM) en el Instituto IES Julio Verne.
Me apasiona la programación, la tecnología y la resolución de problemas. Me considero una persona perfeccionista, competitiva y con muchas ganas de aprender.
Me adapto fácilmente a nuevos entornos y disfruto trabajando en equipo para alcanzar objetivos comunes.
____________________________________________________________________________________________________________________________________________________________________________________
*Tecnologías y herramientas:*
Lenguajes: Java, Kotlin, HTML5, CSS, JavaScript, TypeScript, SQL, XML, JSON, JSX, Python.
Frameworks y librerias: Bootstrap, Angular, React, Spring Boot.
Control de versiones: Git / GitHub.
Bases de datos: MySQL, Oracle, H2, FireBase, Room.
Otros: Desarrollo de Apps con Ionic y uso de ORM como JPA Hibernate.
____________________________________________________________________________________________________________________________________________________________________________________
*Habilidades personales:*
Resolución de problemas,
Pensamiento lógico y analítico,
Adaptabilidad,
Organización y responsabilidad,
Aprendizaje continuo,
Empatía,
Actitud positiva,
Resiliencia
____________________________________________________________________________________________________________________________________________________________________________________
*Contacto*
Correo: omegalex18@gmail.com | Ubicación: San José de la Rinconada (Sevilla), España
